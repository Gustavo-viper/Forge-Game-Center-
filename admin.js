...
async function login(){
  ...
  if(error)return setAuthMessage('Não foi possível entrar: '+friendlyAuthError(error),true);
  await initAdmin();
}
async function signup(){
  ...
  if(error)return setAuthMessage('Não foi possível criar a conta: '+friendlyAuthError(error),true);
  if(!data.user)return setAuthMessage('O Supabase não retornou o usuário criado.',true);
  if(!data.session){
    return setAuthMessage('✓ Conta criada. O Supabase exige confirmação do e-mail. Confirme a mensagem recebida e depois clique em Entrar. Após o login, o botão “Ativar primeiro administrador” aparecerá automaticamente.');
  }
  setAuthMessage('✓ Conta criada e sessão iniciada. Verificando administrador...');
  await initAdmin();
}
function friendlyAuthError(error){
  const m=(error?.message||'').toLowerCase();
  if(m.includes('user already registered')) return 'Este e-mail já possui uma conta. Use “Entrar”.';
  if(m.includes('password')) return 'A senha precisa atender aos requisitos configurados no Supabase.';
  if(m.includes('rate limit')) return 'Limite temporário de tentativas de e-mail atingido. Aguarde alguns minutos e tente novamente.';
  if(m.includes('email')) return error.message;
...
async function claimFirstAdmin(){
  ...
  if(data){setAuthMessage('✓ Primeiro administrador ativado com sucesso.');await initAdmin()}
