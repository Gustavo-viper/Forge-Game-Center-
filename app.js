...
(() => {
  ...
  const defaults = {
    ...
    hangman:{name:'Hangman Pro',status:'available',url:'https://hangman-pro-jbdl.onrender.com'},
    ...
    try{
      ...
      if(supabaseClient){
        const [gamesResult, settingsResult] = await Promise.all([
          supabaseClient
            .from('forge_game_status')
            .select('game_id,name,status,url')
            .order('game_id'),
          supabaseClient
            .from('forge_site_settings')
            .select('id,maintenance_enabled,maintenance_title,maintenance_message,maintenance_return_at,maintenance_button')
            .eq('id','main')
            .maybeSingle()
        ]);
        ...
        gameRows = await restGet('forge_game_status?select=game_id,name,status,url&order=game_id');
        settingRows = await restGet('forge_site_settings?select=id,maintenance_enabled,maintenance_title,maintenance_message,maintenance_return_at,maintenance_button&id=eq.main');
      }
